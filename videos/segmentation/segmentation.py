''' The goal of this file is to segment video frames in order to separate the foreground from the background.
    As i analyse the video, the background is fix and can slighly vary due to shadows.
    
    In order to produce much more accurate results, one would consider the matting problem.
    '''
...
class ProcessingMethod(Enum):
    '''Enum listing the kind of processing methods supported'''
    SIMPLE = 0

    @staticmethod
    def EnumFromString(method):
        '''Convert a given string to enum'''
...
def simpleProcessing(frames):
    '''This is a simple implementation of segmentation. The main assumption made is that the video starts with some empty frames, which are related here as the background'''
    # gray frames all 3 channels 
    g_frames = [np.dstack((cv2.cvtColor(f, cv2.COLOR_BGR2GRAY), cv2.cvtColor(f, cv2.COLOR_BGR2GRAY), cv2.cvtColor(f, cv2.COLOR_BGR2GRAY))) for f in frames]
...
def ProcessVideo(filename, destFile, method):
    ...
    try:
        ...
        ### Insert further processing methods here####
...
def main(args):
    ...
    target_p = dir_name + str('/') + only_fname.split('.')[0] + str('_processed.mp4') if args.video_target_path == None else args.video_target_path
    method = ProcessingMethod.SIMPLE if args.method == None else ProcessingMethod.EnumFromString(args.method)
